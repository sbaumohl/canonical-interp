...
def monomial_forward_loss(pred, y):
    return (pred**2).mean()
...
def test_monomial_rlct():
    ...
    for k in [1, 2, 3]:
        ...
        llc = est.estimate_llc(model, loader, monomial_forward_loss, seed=42, devices=[t.device("cuda")])
...
def test_reduced_rank_regression():
    ...
    for true_rank in [1, 2, 3]:
        ...
        llc = est.estimate_llc(model, loader, rrr_forward_loss, seed=42, devices=t.device("cuda"))
        mean_llc = llc.mean().item()
...
def test_compare_accuracy():
    ...
    for k in [1, 2, 3]:
        ...
        our_llc = est.estimate_llc(model, loader, monomial_forward_loss, seed=42).mean().item()
    ...
    for true_rank in [1, 2, 3]:
        ...
        our_llc = est.estimate_llc(model, rrr_loader, rrr_forward_loss, seed=42).mean().item()
...
def test_speed():
    ...
    for label, H, K, r_max, true_rank, n_samples, batch_size in configs:
        ...
        est.estimate_llc(
            model_ours, loader, rrr_forward_loss, seed=42,
            devices="cuda",
        )
        t.cuda.synchronize()
